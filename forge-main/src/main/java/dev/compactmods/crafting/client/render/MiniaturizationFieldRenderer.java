...
public class MiniaturizationFieldRenderer {
    ...
    public static void onRenderStage(RenderLevelStageEvent evt) {
        if (evt.getStage() == RenderLevelStageEvent.Stage.AFTER_TRANSLUCENT_BLOCKS) {
            ...
            final var partialTicks = evt.getPartialTick();
    ...
    public static void render(Level level, IMiniaturizationField field, float partialTicks, PoseStack pose, MultiBufferSource.BufferSource buffers) {
        ...
        pose.pushPose();
        {
            ...
            if(field.getCraftingState() == EnumCraftingState.CRAFTING && field.getCurrentRecipe().isPresent()) {
                pose.pushPose();
                pose.translate(field.getCenter().getX(), field.getCenter().getY(), field.getCenter().getZ());
                CraftingPreviewRenderer.render(field.getCurrentRecipe().get(), field.getProgress(), pose, buffers, 0, 0);
                pose.popPose();
            }
    ...
    private static void drawProjectorArcs(FieldProjectorEntity tile, PoseStack mx, MultiBufferSource.BufferSource buffers, AABB fieldBounds, double gameTime) {
        ...
        try {
            ...
            builder.vertex(p, (float) projectorCenter.x, (float) projectorCenter.y + 0.2f, (float) projectorCenter.z)
     ...
     */
    private static void drawMainField(Level level, PoseStack mx, MultiBufferSource.BufferSource buffers, IMiniaturizationField field) {
